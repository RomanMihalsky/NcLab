...
class ValidatorNameTest {
  ...
  @BeforeEach
  void init(){
    validatorName = new ValidatorName();
  ...
  void validateOk() throws NoSuchFieldException, IllegalAccessException {
    PatternError patternError = validatorName.validate(contract);
  ...
  void validateError() throws NoSuchFieldException, IllegalAccessException {
    Field fieldDate = client.getClass().getDeclaredField("name");
    fieldDate.setAccessible(true);

    fieldDate.set(client,"sad1212");
