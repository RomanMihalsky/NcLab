...
class ValidatorPassportNumberTest {

  ValidatorPassportNumber validatorPassportNumber;
  Client client;
  Contract contract;

  @BeforeEach
  void init(){
    validatorPassportNumber = new ValidatorPassportNumber();
  ...
  void validateOk() throws NoSuchFieldException, IllegalAccessException {
    PatternError patternError = validatorPassportNumber.validate(contract);
  ...
  void validateError() throws NoSuchFieldException, IllegalAccessException {
    Field fieldDate = client.getClass().getDeclaredField("passportNumber");
