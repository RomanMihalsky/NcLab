...
class ValidatorSurnameTest {

  ValidatorSurname validatorSurname;
  Client client;
  Contract contract;

  @BeforeEach
  void init(){
    validatorSurname = new ValidatorSurname();
  ...
  void validateOk() throws NoSuchFieldException, IllegalAccessException {
    PatternError patternError = validatorSurname.validate(contract);
  ...
  void validateError() throws NoSuchFieldException, IllegalAccessException {
    Field fieldDate = client.getClass().getDeclaredField("surname");
