...
class ValidatorPhoneNumberTest {
  ...
  @BeforeEach
  void init(){
    validatorPhoneNumber = new ValidatorPhoneNumber();
  ...
  void validateOk() throws NoSuchFieldException, IllegalAccessException {
    PatternError patternError = validatorPhoneNumber.validate(contract);
  ...
  void validateError() throws NoSuchFieldException, IllegalAccessException {
    Field fieldDate = client.getClass().getDeclaredField("phoneNumber");
