...
class ValidatorPassportSeriesTest {

  ValidatorPassportSeries validatorPassportSeries;
  Client client;
  Contract contract;

  @BeforeEach
  void init(){
    validatorPassportSeries = new ValidatorPassportSeries();
  ...
  void validateOk() throws NoSuchFieldException, IllegalAccessException {
    PatternError patternError = validatorPassportSeries.validate(contract);
  ...
  void validateError() throws NoSuchFieldException, IllegalAccessException {
    Field fieldDate = client.getClass().getDeclaredField("passportSeries");
